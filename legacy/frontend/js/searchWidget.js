...
export function initSearchWidget() {
    ...
    if (addFlightBtn && multicityForm) {
        ...
        // Auto-fill origin of next flight with destination of current flight
        if (multicityForm) {
            multicityForm.addEventListener('input', (e) => {
                if (e.target.classList.contains('m-to')) {
                    const row = e.target.closest('.multicity-row');
                    const allRows = Array.from(multicityForm.querySelectorAll('.multicity-row'));
                    const index = allRows.indexOf(row);
                    
                    if (index !== -1 && index < allRows.length - 1) {
                        const nextRow = allRows[index + 1];
                        const nextFrom = nextRow.querySelector('.m-from');
                        if (nextFrom && !nextFrom.dataset.manualEdit) {
                            nextFrom.value = e.target.value;
                        }
                    }
                } else if (e.target.classList.contains('m-from')) {
            ...
            for (let i = 0; i < flightsArray.length; i++) {
                ...
                const response = await fetch(url, {
                    ...
                    body: JSON.stringify({
                        ...
                        to: destCode,
                        departDate: f.date,
                        tripType: 'oneway' // Processed sequentially for now
                    }),
                    mode: 'cors'
                });
